 ...
 *
 *
 * 此类可以灵活的控制log输出,解决了因不同的人开发的时候打印不同的log造成程序log打印非常混乱,只需做一些简单的配置
...
public class LogUtil {
    ...
    private static void log(LEVEL level, StackTraceElement[] sElements, String... msg) {
        if (!DEBUG) {
        ...
        if (check(className)) {
            print(level, simpleClassName, methodName, lineNumber, msg);
        } else {
//            if (properties == null) {
//                print(level, msg);
//            }
    ...
    private static void print(LEVEL level, String simpleClassName, String methodName, int lineNumber, String... msg) {
        String tag = "";
        String text = "";
    ...
    private static void saveLogToFile(LEVEL level, StackTraceElement[] sElements, String... msg) {
        if (!DEBUG) {
            return;
        }
        saveLogToFile(level, sElements, saveUrl, DateFormatUtil.format(DateFormatUtil.FORMAT_2) + ".txt", false, msg);
    }
     ...
     * @param msg
     */
    private static void saveLogToFile(LEVEL level, StackTraceElement[] sElements, String dir, String fileName, boolean clear, String... msg) {
        String className = sElements[1].getClassName();
        String simpleClassName = className;
        int i = className.lastIndexOf(".");
        if (i + 1 < className.length() - 1) {
    ...
    private static String formatTag(String simpleClassName, String methodName, int lineNumber) {
    ...
    public static void test(String... msg) {
        ...
        log(LEVEL.error, stackTraceElements, msg);
        saveLogToFile(LEVEL.debug, stackTraceElements, testUrl, DateFormatUtil.format(DateFormatUtil.FORMAT_2) + ".txt", true, msg);
    }

    public static void v(String... msg) {
        StackTraceElement[] stackTraceElements = new Throwable().getStackTrace();
        log(LEVEL.verbose, stackTraceElements, msg);
        if (saveFile && v) {
            saveLogToFile(LEVEL.verbose, stackTraceElements, msg);
        }
    }

    public static void d(String... msg) {
        StackTraceElement[] stackTraceElements = new Throwable().getStackTrace();
        log(LEVEL.debug, stackTraceElements, msg);
        if (saveFile && d) {
            saveLogToFile(LEVEL.debug, stackTraceElements, msg);
        }
    }

    public static void i(String... msg) {
        StackTraceElement[] stackTraceElements = new Throwable().getStackTrace();
        log(LEVEL.info, stackTraceElements, msg);
        if (saveFile && i) {
            saveLogToFile(LEVEL.info, stackTraceElements, msg);
        }
    }

    public static void w(String... msg) {
        StackTraceElement[] stackTraceElements = new Throwable().getStackTrace();
        log(LEVEL.warn, stackTraceElements, msg);
        if (saveFile && w) {
            saveLogToFile(LEVEL.warn, stackTraceElements, msg);
        }
    }
    ...
    public static void e(String... msg) {
        ...
        log(LEVEL.error, stackTraceElements, msg);
        if (saveFile && e) {
            saveLogToFile(LEVEL.error, stackTraceElements, msg);
        }
    }
    ...
    public static void e(Throwable throwable) {
        ...
        log(LEVEL.error, stackTraceElements, stringWriter.toString());
        if (saveFile && e) {
            saveLogToFile(LEVEL.error, stackTraceElements, stringWriter.toString());
        }
    }
    ...
    public static void e(String tag, Exception throwable) {
        ...
        PrintWriter printWriter = new PrintWriter(stringWriter);
        throwable.printStackTrace(printWriter);
        log(LEVEL.error, stackTraceElements, tag, stringWriter.toString());
        if (saveFile && e) {
            saveLogToFile(LEVEL.error, stackTraceElements, stringWriter.toString());
        }
    }
